...
PYTHON_UNSAFE_ATTRS = {
    ("pickle", "loads"): ("high", "Unsafe deserialization via pickle.loads().", "Use JSON or a safe serialization format."),
    ("pickle", "load"): ("high", "Unsafe deserialization via pickle.load().", "Use JSON or a safe serialization format."),
...
JS_PATTERNS: list[tuple[str, re.Pattern, str, str, str]] = [
    ("unsafe-eval", re.compile(r"\beval\s*\("), "high", "eval() can execute arbitrary code.", "Remove eval() or use a safe alternative."),
    ("unsafe-innerhtml", re.compile(r"\.innerHTML\s*="), "high", "Direct innerHTML assignment enables XSS.", "Use textContent or a sanitization library."),
    ("unsafe-react-html", re.compile(r"dangerouslySetInnerHTML"), "high", "dangerouslySetInnerHTML bypasses React XSS protection.", "Sanitize HTML with DOMPurify before rendering."),
    ("unsafe-child-process", re.compile(r"child_process.*exec\b"), "high", "Unsanitized shell command execution.", "Use execFile() with explicit args instead."),
    ("sql-concat", re.compile(r"""(?:query|execute)\s*\(\s*[f'"`].*\$\{|(?:query|execute)\s*\(\s*['"].*['"]\s*\+"""), "high", "SQL string concatenation risks injection.", "Use parameterized queries."),
]
...
class _UnsafeCallVisitor(ast.NodeVisitor):
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if isinstance(node.func, ast.Attribute) and node.func.attr in ("run", "call", "Popen", "check_output", "check_call"):
