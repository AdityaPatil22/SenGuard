...
SECRET_PATTERNS: list[tuple[str, re.Pattern, str]] = [
    ...
    (
        ...
        "Rotate this AWS secret key.",
    ),
    (
        "GitHub Token",
        re.compile(r"(ghp_[A-Za-z0-9]{36}|gho_[A-Za-z0-9]{36}|ghs_[A-Za-z0-9]{36}|github_pat_[A-Za-z0-9_]{22,})"),
        "Revoke and regenerate this GitHub token.",
    ),
    ...
    (
        ...
        "Remove private key from source code.",
    ),
    (
        "Generic Secret Assignment",
        re.compile(r"""(?i)(?:password|passwd|secret|token|api_key|apikey|api-key)\s*[=:]\s*['"]([^'"]{8,})['"]"""),
        "Move this secret to environment variables.",
    ),
    (
        "Connection String",
        re.compile(r"(?:postgres(?:ql)?|mysql|mongodb(?:\+srv)?|redis)://[^\s'\"]{10,}"),
